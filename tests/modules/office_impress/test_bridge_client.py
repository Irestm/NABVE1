...
from modules.office_impress import bridge_client as bridge_client_module
from modules.office_impress.bridge_client import (
    DEFAULT_COMMAND_TIMEOUT_SECONDS,
    OfficeImpressBridgeClient,
    OfficeImpressUnavailableError,
    OPEN_PRESENTATION_TIMEOUT_SECONDS,
)
...
async def test_is_bridge_running_true_on_200() -> None:
    ...
    client = OfficeImpressBridgeClient(host="127.0.0.1", port=8767)
...
async def test_ensure_bridge_running_raises_when_script_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(OfficeImpressUnavailableError):
        await client.ensure_bridge_running()
...
async def test_send_command_returns_payload_on_success() -> None:
    ...
    result = await client.send_command("set_slide_title", {"text": "hi"})

    assert result == {"status": "success", "message": "", "data": {}}
    assert _FakeAsyncClient.last_post_url == "http://127.0.0.1:8767/command"
    assert _FakeAsyncClient.last_post_json == {"action": "set_slide_title", "params": {"text": "hi"}}
...
async def test_send_command_uses_default_timeout_for_ordinary_actions() -> None:
    ...
    await client.send_command("set_slide_title", {})

    assert _FakeAsyncClient.last_timeout == DEFAULT_COMMAND_TIMEOUT_SECONDS


@pytest.mark.asyncio
async def test_send_command_uses_longer_timeout_for_open_presentation() -> None:
    _FakeAsyncClient.post_response = _FakeResponse(json_body={"status": "success"})
    client = OfficeImpressBridgeClient(host="127.0.0.1", port=8767)

    await client.send_command("open_presentation", {})

    assert _FakeAsyncClient.last_timeout == OPEN_PRESENTATION_TIMEOUT_SECONDS
