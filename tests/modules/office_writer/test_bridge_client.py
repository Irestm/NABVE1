...
from modules.office_writer import bridge_client as bridge_client_module
from modules.office_writer.bridge_client import (
    DEFAULT_COMMAND_TIMEOUT_SECONDS,
    OfficeWriterBridgeClient,
    OfficeWriterUnavailableError,
    OPEN_DOCUMENT_TIMEOUT_SECONDS,
)
...
class _FakeScriptPath:
    ...
    def __str__(self) -> str:
        return "/fake/office_bridge/writer_server.py"
...
async def test_is_bridge_running_true_on_200() -> None:
    ...
    client = OfficeWriterBridgeClient(host="127.0.0.1", port=8767)
...
async def test_ensure_bridge_running_raises_when_script_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(OfficeWriterUnavailableError):
        await client.ensure_bridge_running()
...
async def test_ensure_bridge_running_spawns_server_win_with_this_interpreter_on_windows(
    ...
    # _IS_WINDOWS/_BRIDGE_SCRIPT are computed once at import time from the
    # real platform.system() (False/server.py here, since tests run on
    # Linux) — monkeypatching platform.system() at test time wouldn't
    # retroactively change them, so this simulates "if this had been
    # computed on Windows" by patching both directly, same as
    # _FakeScriptPath already does for the "script missing" case above.
    import sys

    responses = iter(
        [
            httpx.ConnectError("refused"),  # initial check: not up yet
            _FakeResponse(status_code=200),  # first poll after spawning: up
        ]
    )
...
async def test_send_command_returns_payload_on_success() -> None:
    ...
    result = await client.send_command("insert_text", {"content": "hi"})

    assert result == {"status": "success", "message": "", "data": {}}
    assert _FakeAsyncClient.last_post_url == "http://127.0.0.1:8767/command"
    assert _FakeAsyncClient.last_post_json == {"action": "insert_text", "params": {"content": "hi"}}
...
async def test_send_command_uses_default_timeout_for_ordinary_actions() -> None:
    ...
    await client.send_command("insert_text", {})

    assert _FakeAsyncClient.last_timeout == DEFAULT_COMMAND_TIMEOUT_SECONDS


@pytest.mark.asyncio
async def test_send_command_uses_longer_timeout_for_open_document() -> None:
    _FakeAsyncClient.post_response = _FakeResponse(json_body={"status": "success"})
    client = OfficeWriterBridgeClient(host="127.0.0.1", port=8767)

    await client.send_command("open_document", {})

    assert _FakeAsyncClient.last_timeout == OPEN_DOCUMENT_TIMEOUT_SECONDS
