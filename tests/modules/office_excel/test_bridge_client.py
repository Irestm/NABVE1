...
from modules.office_excel import bridge_client as bridge_client_module
from modules.office_excel.bridge_client import (
    DEFAULT_COMMAND_TIMEOUT_SECONDS,
    OfficeExcelBridgeClient,
    OfficeExcelUnavailableError,
    OPEN_SPREADSHEET_TIMEOUT_SECONDS,
)
...
async def test_is_bridge_running_true_on_200() -> None:
    ...
    client = OfficeExcelBridgeClient(host="127.0.0.1", port=8767)
...
async def test_ensure_bridge_running_raises_when_script_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(OfficeExcelUnavailableError):
        await client.ensure_bridge_running()
...
async def test_send_command_returns_payload_on_success() -> None:
    ...
    result = await client.send_command("set_cell_value", {"cell": "A1", "value": "hi"})

    assert result == {"status": "success", "message": "", "data": {}}
    assert _FakeAsyncClient.last_post_url == "http://127.0.0.1:8767/command"
    assert _FakeAsyncClient.last_post_json == {"action": "set_cell_value", "params": {"cell": "A1", "value": "hi"}}
...
async def test_send_command_uses_default_timeout_for_ordinary_actions() -> None:
    ...
    await client.send_command("set_cell_value", {})

    assert _FakeAsyncClient.last_timeout == DEFAULT_COMMAND_TIMEOUT_SECONDS


@pytest.mark.asyncio
async def test_send_command_uses_longer_timeout_for_open_spreadsheet() -> None:
    _FakeAsyncClient.post_response = _FakeResponse(json_body={"status": "success"})
    client = OfficeExcelBridgeClient(host="127.0.0.1", port=8767)

    await client.send_command("open_spreadsheet", {})

    assert _FakeAsyncClient.last_timeout == OPEN_SPREADSHEET_TIMEOUT_SECONDS
