"""Backend-side client for the Jarvis <-> LibreOffice bridge, Impress side.

Talks to the SAME shared bridge process as modules/office_writer/ and
modules/office_excel/'s bridge_client.py (office_bridge/server.py — one
soffice/UNO connection for every LibreOffice app). Its own exception type
mirrors OfficeWriterUnavailableError/OfficeExcelUnavailableError so callers
never have to guess which app a bridge failure was about."""
...
# "open_presentation" may have to launch soffice cold and wait for its UNO
...
OPEN_PRESENTATION_TIMEOUT_SECONDS = 45.0
...
class OfficeImpressUnavailableError(Exception):
    """Raised for every failure mode a caller should report as "the Impress
...
class OfficeImpressBridgeClient:
    ...
    async def ensure_bridge_running(self) -> None:
        ...
        answer, and waits for it to come up. If Writer or Calc already
        launched the bridge, /health already answers here and this is a
        no-op — one shared process serves all three."""
        if await self.is_bridge_running():
            return
        if not _BRIDGE_SCRIPT.exists():
            raise OfficeImpressUnavailableError(f"Файл моста LibreOffice не найден: {_BRIDGE_SCRIPT}")

        logger.info("office bridge not running, launching it (from office_impress)")
        try:
            subprocess.Popen(
                [
            ...
            raise OfficeImpressUnavailableError(f"Не удалось запустить мост LibreOffice: {exc}") from exc
        ...
        raise OfficeImpressUnavailableError(
    ...
    async def send_command(
        ...
        {status, message, data} reply. Raises OfficeImpressUnavailableError
        for every transport failure; a command the bridge understood but
        rejected (e.g. "no presentation open") still comes back as a normal
        {"status": "error", "message": "..."} payload — see
        modules/office_impress/dispatcher.py for how those two cases are
        told apart."""
        effective_timeout = timeout if timeout is not None else (
            OPEN_PRESENTATION_TIMEOUT_SECONDS if action == "open_presentation" else DEFAULT_COMMAND_TIMEOUT_SECONDS
        )
        try:
            ...
            raise OfficeImpressUnavailableError(f"Не удалось достучаться до моста LibreOffice: {exc}") from exc

        try:
            payload = response.json()
        except ValueError as exc:
            raise OfficeImpressUnavailableError("Мост LibreOffice вернул не-JSON ответ") from exc

        if not isinstance(payload, dict) or "status" not in payload:
            raise OfficeImpressUnavailableError("Мост LibreOffice вернул ответ неожиданной формы")
        return payload


office_impress_bridge_client = OfficeImpressBridgeClient()
