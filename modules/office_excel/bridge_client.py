"""Backend-side client for the Jarvis <-> LibreOffice bridge, Calc side.

Talks to the SAME shared bridge process as modules/office_writer/bridge_client.py
(office_bridge/server.py — one soffice/UNO connection for every LibreOffice
app, see that script's own docstring) rather than a second one; this class
exists as its own copy instead of a shared base purely so each app's own
"is the bridge unreachable" exception type stays distinct for callers
(modules/office_excel/dispatcher.py never has to guess whether an
OfficeWriterUnavailableError means Calc is unreachable too)."""
...
# "open_spreadsheet" may have to launch soffice cold and wait for its UNO
...
OPEN_SPREADSHEET_TIMEOUT_SECONDS = 45.0
...
class OfficeExcelUnavailableError(Exception):
    """Raised for every failure mode a caller should report as "the Calc
...
class OfficeExcelBridgeClient:
    ...
    async def ensure_bridge_running(self) -> None:
        ...
        answer, and waits for it to come up. If modules/office_writer already
        launched the bridge for a Writer command, /health already answers
        here and this is a no-op — one shared process serves both."""
        if await self.is_bridge_running():
            return
        if not _BRIDGE_SCRIPT.exists():
            raise OfficeExcelUnavailableError(f"Файл моста LibreOffice не найден: {_BRIDGE_SCRIPT}")

        logger.info("office bridge not running, launching it (from office_excel)")
        try:
            subprocess.Popen(
                [
            ...
            raise OfficeExcelUnavailableError(f"Не удалось запустить мост LibreOffice: {exc}") from exc
        ...
        raise OfficeExcelUnavailableError(
    ...
    async def send_command(
        ...
        {status, message, data} reply. Raises OfficeExcelUnavailableError for
        every transport failure; a command the bridge understood but
        rejected (e.g. "no spreadsheet open") still comes back as a normal
        {"status": "error", "message": "..."} payload — see
        modules/office_excel/dispatcher.py for how those two cases are told
        apart."""
        effective_timeout = timeout if timeout is not None else (
            OPEN_SPREADSHEET_TIMEOUT_SECONDS if action == "open_spreadsheet" else DEFAULT_COMMAND_TIMEOUT_SECONDS
        )
        try:
            ...
            raise OfficeExcelUnavailableError(f"Не удалось достучаться до моста LibreOffice: {exc}") from exc

        try:
            payload = response.json()
        except ValueError as exc:
            raise OfficeExcelUnavailableError("Мост LibreOffice вернул не-JSON ответ") from exc

        if not isinstance(payload, dict) or "status" not in payload:
            raise OfficeExcelUnavailableError("Мост LibreOffice вернул ответ неожиданной формы")
        return payload


office_excel_bridge_client = OfficeExcelBridgeClient()
